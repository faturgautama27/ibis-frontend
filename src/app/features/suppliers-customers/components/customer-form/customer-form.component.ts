...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, ButtonModule, InputTextModule, TextareaModule, CheckboxModule, MessageModule, LucideAngularModule],
  template: `
    <div class="main-layout overflow-hidden">
      ...
      <div class="mb-6">
        <div class="flex items-center gap-2 mb-2">
          ...
          <h1 class="text-2xl font-semibold text-gray-900">{{ isEditMode ? 'Edit Customer' : 'Create Customer' }}</h1>
        </div>
      ...
      <div class="bg-white rounded-lg shadow-sm p-6" style="max-height: calc(100vh - 13rem); overflow-y: auto">
        <form [formGroup]="customerForm" (ngSubmit)="onSubmit()" class="space-y-6">
          ...
          <div class="mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div class="flex flex-col">
                <label class="text-sm font-medium text-gray-700 mb-1">Customer Code <span class="text-red-500">*</span></label>
                <input pInputText formControlName="customer_code" class="w-full" />
                <small *ngIf="isFieldInvalid('customer_code')" class="text-red-600 mt-1">Customer code is required</small>
              </div>
              <div class="flex flex-col">
                <label class="text-sm font-medium text-gray-700 mb-1">Customer Name <span class="text-red-500">*</span></label>
              ...
              <div class="flex flex-col md:col-span-2">
                <label class="text-sm font-medium text-gray-700 mb-1">Address <span class="text-red-500">*</span></label>
              ...
              <div class="flex flex-col">
                <label class="text-sm font-medium text-gray-700 mb-1">Tax ID (NPWP) <span class="text-red-500">*</span></label>
                <input pInputText formControlName="tax_id" placeholder="XX.XXX.XXX.X-XXX.XXX" (blur)="onNPWPBlur()" class="w-full font-mono" />
                <small *ngIf="isFieldInvalid('tax_id')" class="text-red-600 mt-1">{{ customerForm.get('tax_id')?.hasError('required') ? 'Tax ID is required' : 'Invalid NPWP format' }}</small>
          ...
          <div class="flex justify-end gap-3 pt-4 border-t border-gray-200">
            ...
            <button pButton type="submit" [label]="isEditMode ? 'Update' : 'Create'" icon="pi pi-check" [loading]="loading" [disabled]="customerForm.invalid || loading"></button>
          </div>
        </form>
      </div>
    </div>
